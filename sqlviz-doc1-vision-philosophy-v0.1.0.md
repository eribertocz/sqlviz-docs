...
### Rule 7 — One Package, One Responsibility

Each package does exactly one thing.
If a package is doing two things, it should be two packages.

```
sqlviz-parser    → only parses SQL
sqlviz-inference → only infers analytical intent
sqlviz-storage   → only manages persistence
sqlviz-api       → only handles HTTP
```

No package knows what another package does.
...
```
RuntimeContext    The immutable object that flows through
