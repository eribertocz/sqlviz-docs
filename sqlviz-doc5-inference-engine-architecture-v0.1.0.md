...
│    → Complete Feature Vector V0 (38 dims)           │
└─────────────────────┬───────────────────────────────┘
                      │ Feature Vector [38]
                      ▼
┌─────────────────────────────────────────────────────┐
...
│    → Semantic Features (dims 30-37)                 │
│    → Complete Feature Vector V0 (38 dims) final     │
└─────────────────────┬───────────────────────────────┘
...
### 1.3 The 7 Modules and How They Connect

```
Module              Input                   Output
────────────────────────────────────────────────────────────────
Parser              SQL string              AST + Fingerprint
Feature Engine      AST + data + schema     FeatureVector[38]
Semantic Engine     FeatureVector + schema  FeatureVector[38] enriched
Intent Engine       FeatureVector[38]       IntentVector
...
features/feature_engine.py  Coordinates dims 0-37.
...
feature_engine.run(context)      ← adds: feature_vector complete
    ↓
semantic_engine.run(context)     ← enriches: feature_vector dims 30-37
    ↓
...
class RuntimeContext:
    ...
    Modules read from context and return enriched context.
    Never mutate — always return a new enriched instance.
    """
    ...
    feature_vector: list[float] = field(
        default_factory=lambda: [0.0] * 38
    )
    # complete 38-dimension feature vector V0
...
class IntentEngine:
    def run(self, context: RuntimeContext) -> RuntimeContext:
        try:
            ...
            # Never mutate context — always construct new fields
...
Rule 1 — Modules only write to their own fields
    ...
    Semantic Engine: feature_vector (dims 30-37 only)
...
feature_vector      [0.0] * 38      neutral — no features
...
class SQLParser:
    ...
    def _parse(self, context: RuntimeContext) -> RuntimeContext:
        ...
        fv = context.feature_vector  # [0.0] * 38
...
class TestSQLFeatures:
    ...
    def test_invalid_sql_graceful(self):
        ...
        assert ctx.fingerprint == "UNKNOWN"
        assert ctx.feature_vector == [0.0] * 38
...
The Feature Engine computes the complete 38-dimension
...
class FeatureEngine:
    """
    Computes the complete 38-dimension feature vector V0.
...
using the complete 38-dimension feature vector.
...
def make_result(chart_winner: str, intent_winner: str, col_span: int) -> InferenceResult:
    ...
    return InferenceResult(
        ...
        feature_vector=[0.0]*38, errors=[], elapsed_ms=0.0
    )
...
✅ Section 16 — v0.1.1 Patch: critical fixes + V0.2/V0.3 tracking
```
...
→ Expand Feature Vector V0 (38 dims) toward V1 (80-120 dims)
...
─────────────────────────────────────────────────────────────────────────
Feature Vector            38 dims, list[float]   → 80-120 dims (V1)  PLANNED
                                                  → 150+ dims (V2)
                          Rule: always append,
...
*SQLviz Inference Engine Architecture — v0.1.0*
