...
DOC 6 → UI Design System            (the face)         [pending]
DOC 7 → Security & Roles            (the lock)         [pending]
...
│   │
│   ├── sqlviz-web/
│   │   └── src/                SvelteKit — structure in DOC 6
│   │
│   └── sqlviz-cli/
│       └── src/sqlviz_cli/
...
2. routers/auth.py — admin login (DOC 2, Section 5;
   full design in DOC 7, pending)
3. routers/shares.py — share link generation/verification
   (DOC 2, Section 6; full design in DOC 7, pending)
...
**Note:** Phase 4 depends on DOC 7 (Security & Roles) for full
auth/share correctness. If DOC 7 is not yet written when Phase 4
starts, implement auth/shares using the DOC 2 Section 5-6 design
as the interim source of truth, then revisit once DOC 7 exists.
This is explicitly allowed — DOC 7 refines, it does not block.
...
Tasks: (full detail pending DOC 6 — UI Design System)
...
5. Admin login screen, viewer share-link screen
   (interim per DOC 2 until DOC 7 exists)
...
─────────────────────────────────────────────────────────
Auto-layout v0.2 UX               DOC 1 vision          V0.2
 ("no rows, no drag, just SQL")
